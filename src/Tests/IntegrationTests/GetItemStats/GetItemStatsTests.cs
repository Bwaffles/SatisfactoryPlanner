...
internal class GetItemStatsTests : IntegrationTest
{
    ...
    private async Task<ItemStatsResult> GetItemStats(Guid worldId) => await Polling.GetEventually(new GetItemStatsProbe(WarehousesModule, worldId), 7000);
    ...
    public class GetItemStatsProbe(IWarehousesModule warehousesModule, Guid worldId) : IProbe<ItemStatsResult>
    {
        private readonly IWarehousesModule _warehousesModule = warehousesModule;
        private readonly Guid _worldId = worldId;

        public async Task<bool> IsSatisfiedAsync(ItemStatsResult sample) => await Task.Run(() => sample.Items.Count > 0);
