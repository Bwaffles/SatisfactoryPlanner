    ...
    internal class ExtractorFactory
    {
        public static async Task<Extractor> GetExtractor(IDbConnection connection, Guid extractorId)
        {
            ...
            return Extractor.CreateNew(
                ...
                ExtractorClockspeed.CreateNew(extractor.DefaultClockspeed, extractor.OverclockPerShard, extractor.MaxShards),
                allowedResources.Select(_ => new ResourceId(_)).ToList());
        }

        internal static async Task<List<ExtractorDto>> GetAvailableExtractors(IDbConnection connection, Guid? resourceId)
        {
        ...
        internal static async Task<List<Guid>> GetAllowedResources(IDbConnection connection, Guid extractorId)
        {
